...
- 갱신: Rust 기반 이후 R1 setup 입력·수정·checkpoint/재개·새 제품 설정 출력, R2 새 폴더 installer, R3 engine-info/cold·등록·명시 init/resume-init·수동 LaunchAgent start/status/stop을 구현했다. setup의 package 선택·자동 연결과 전체 G1-M은 후속이다.
...
아래 표는 전체 목표 UX다. R1은 2~4단계의 제품 설정 초안과 로컬 metadata 검사 부분을 제공한다. 독립 install·cold·instance 등록·init/resume-init·start/status/stop 명령도 구현했다. package 선택·등록을 setup에 연결하기, 콘솔 연결·제거는 후속이다. 후속 운영 조회는 기존 NIGO의 로컬 브라우저 콘솔을 재사용한다.
...
뒤로 가기·다시 실행하기는 파일 덮어쓰기나 새 genesis 생성의 동의가 아니다. 입력 답은 제품 형식 규칙을 검사한 뒤 0600 checkpoint로 저장하며 완성된 초안의 로컬 검사 실패도 보존한다. 누락된 참조 파일이 있어도 초안·설정을 저장할 수 있고 저장 성공은 엔진 준비 완료를 의미하지 않는다. init/start는 별도의 명시 명령이다. `:cancel`·EOF 뒤에는 같은 workspace의 `--resume`으로 마지막 완료 저장부터 이어간다. password 원문을 인자·설정·결과에 넣지 않는다.
...
Rust crate 구성은 `src/{cli,error,artifact,config,setup,install,engine}`이며 instance/service와 launchd·runtime 관측 adapter는 engine 아래 구현했다. diagnostics는 후속이다. setup의 checkpoint·파일 출력은 공통 제품 코드이며 Mac 경로 별칭 충돌을 보수적으로 검사한다. 프로세스 호출은 인자 배열을 사용하고 shell 문자열 연결이나 임의 실행 옵션을 제품 설정으로 받지 않는다.

## 5. launchd 실행·종료·재개

Mac 첫 profile은 사용자 LaunchAgent로 구현한다. 사용자가 로그인한 세션에서 명시 start로 수동 bootstrap하며 부팅 전용 server daemon과 다르다. 최초 설치는 자동 시작하지 않는다. 로그인 자동 시작은 현재 제공하지 않으며 후속 명시 정책·인수 대상으로 남긴다. KeepAlive/자동 재시작은 사용하지 않는다. 현재 gate·종료 정책은 [LaunchAgent 설계](./2026-09-18-macos-launchagent.md)를 따른다.
...
로그아웃·로그인, Mac 잠자기/깨우기, 터미널 종료, GUI 세션 부재, launchd 미등록, 부분 설치·기동 실패를 UX 인수에 포함한다. 노트북의 sleep 동안 네트워크 liveness를 보장하지 않는다. 현재 개발 CLI는 start/status/stop을 제공하며 단일 validator 서비스 시험과 전체 G1-M 인수를 구분한다.
...
| R2 일부 구현 | 검증된 Mac package를 새 폴더에 설치·완료 receipt, 독립 instance 등록 | host/서명/hash/alias 검증·기존 경로 거부. 중단은 완료 receipt 없는 폴더로 남기고 자동 resume하지 않음. setup 연결 후속 |
| R3 일부 구현 | pinned engine-info/cold·명시 init/resume·수동 LaunchAgent start/status/stop | cold INCOMPLETE 보존, 실제 단일 validator 초기화·정상 종료/같은 DB 재시작 확인. 전체 실패·사용자 세션·G1-M 인수 후속 |
...
NIGO `REQ-0002`의 제공 owner는 범위를 수용했고 BXDL은 피드백 3절의 안전·인수 의미와 4·5절의 결과 교환에 동의했다. [소비자 회신](./2026-09-17-nigo-feedback-response.md)에 당시 수신 revision·owner·계약 확인 사항을 기록한다. 최초 dirty 후보 이후 clean source `303e163a`의 API/flag/fixture/JAR와 QBFT 종료/drain 보완 근거를 수신했으며 현재는 [clean 계약 snapshot](../contracts/nigo/development-clean-2026-09-18/README.md)을 소비한다. 실제 Mac 단일 validator의 초기화·LaunchAgent 정상 종료·같은 DB 재시작을 확인했다. 계약은 PROPOSED이며 정식 공급·전체 G1-M 인수가 남아 요청 완료로 올리지 않는다. 최신 범위는 [구현 상태](../docs/implementation-status.md)를 따른다. 이번 BXDL 작업은 공유 중인 NIGO 원장을 수정하지 않으며 엔진 source·사용자 node/data도 변경하지 않는다.
