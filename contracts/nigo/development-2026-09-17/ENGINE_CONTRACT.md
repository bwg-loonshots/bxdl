...
현재 fresh network의 검증 순서는 초기 validator의 `sync.enabled=false` 기동 → 첫 비-genesis 거래 확정 →
observer·뒤처진 validator의 `sync.enabled=true` 기동이다. 동기화 source에는 `sync.archive.enabled=true`가 필요하다.
모든 fresh node가 catch-up부터 기다리는 구성과 finalized checkpoint가 없는 genesis-only observer는 지원 완료가 아니다.
첫 구성은 아직 source archive가 게시되기 전 상호 의존하며, 둘째는 현행 follower의 checkpoint 요구로 거부된다.
live follower의 공급자 단절 후 자동 전환과 단절 상태 반영도 미완료다. 재시작 공급자 시험은 observer의
동기화 공급자가 아닌 validator(`sync.archive.enabled=false`)를 대상으로 하며 source 장애 인수가 아니다.
...
현재 후보에서 QBFT의 follower·sync serving·signer 내부 작업 전체의 종료 증명은 아직 제공하지 않는다.
상위 consensus stop과 storage close가 반환되어도 `UNKNOWN / QBFT_CLEANUP_NOT_FULLY_OBSERVED`로 보고하며,
확인된 `details.storageClosed`, `details.consensusStopReturned`만 분리해 제공한다. 이를 정상 종료 인수로 승격하지 않는다.
...
계약 정의는 [contract.json](src/main/resources/engine/contract.json), 진행·실제 결과는 [작업 기록](../../tasks/2026-09-17-bxdl-engine-foundation.md)을 따른다.
